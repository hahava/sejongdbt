...
public class ExecuteProject {
	...
	private static void showMenu() {
		...
		if (isAdmin()) {
			...
			System.out.println("18. 직원관련 검색"); // 쿼리 5개 > 완료 / 이름기반 검색 (join),
												// 나이 기반 검색(subquery.. <확인바람),
												// 부서 검색 단순쿼리,
												// 근태 직원 (단순쿼리), 최대 최소연봉 지정해서 쿼리
												// (subquery) (완료)

			System.out.println("19. 우수회원"); // 4개 스낵 많이 산 사람, 영화 많이 본 사람
											// (groupby, having 완료_영화)/ 스낵은 진행중
											// 2(비슷한거 썼다고 쳐도 2)

			System.out.println("20. 영화 통계");// 영화 개요 검색(전체 검색> 세 테이블 조인/ 이름 검색 >
											// 단순 조건)(완)
